...
void test("formatRuntime renders minutes and seconds with left padding", () => {
    const result = formatRuntime(182000);

    assert.equal(result, "03m02s");
});

void test("buildRunSummaryLine renders success summary", () => {
    const result = buildRunSummaryLine("hello.py", 0, 182000, null);

    assert.equal(result, "-- Process hello.py succeeded (exit code 0, runtime 03m02s) --");
});
...
void test("buildRunSummaryLine renders failure summary with signal", () => {
    ...
    assert.equal(result, "-- Process fail.py failed (exit code 2, runtime 02m05s, signal SIGTERM) --");
});
